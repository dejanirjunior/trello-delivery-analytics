...
def build_dashboard_html(cliente, df_cliente):
    ...
    df_exec = df_cliente.copy()

    # Criticidade executiva simples
    df_exec["criticidade"] = "Normal"
    df_exec.loc[df_exec["bloqueado"] == True, "criticidade"] = "Bloqueado"
    df_exec.loc[df_exec["risk"].fillna("").astype(str).str.lower().isin(["high", "alto"]), "criticidade"] = "Risco Alto"
    ...
    table_df["effort"] = pd.to_numeric(table_df["effort"], errors="coerce")
    table_df["effort"] = table_df["effort"].fillna(0)
    table_df["bloqueado"] = table_df["bloqueado"].apply(lambda x: "Sim" if bool(x) else "Não")
    table_df["data_de_entrega"] = table_df["data_de_entrega"].fillna("Não informada")
  ...
  <div class="container">
    <section class="kpis">
      ...
      <div class="kpi">
        ...
        <div class="kpi-value">{int(status_counts.get("Done", 0))}</div>
      </div>
    </section>
    ...
    <section class="grid">
      ...
      <div class="panel">
        ...
        <p style="line-height:1.7; font-size:0.95rem;">
          Este painel apresenta a distribuição atual das demandas do cliente, com foco em andamento,
          bloqueios, esforço total e exposição a risco. O objetivo é permitir leitura rápida do cenário
          e facilitar decisões de acompanhamento.
        </p>
      </div>
    </section>
    ...
    <section class="table-panel">
      ...
      <div class="footer-note">
        Dashboard gerado automaticamente a partir da base estruturada do Trello.
      </div>
    </section>
  </div>
