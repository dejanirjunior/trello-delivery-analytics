...
def main():
    df_throughput = pd.read_csv(THROUGHPUT) if THROUGHPUT.exists() else pd.DataFrame(columns=["week", "cards_entregues"])
...
<body>
  <header>
    <div class="header-row">
      <div>
        ...
        <div class="subtitle">Indicadores executivos de fluxo, entrada de demanda, throughput e lead time médio.</div>
      </div>
      <div>
  ...
  <script>
    ...
    new Chart(document.getElementById("backlog"), {{
      type: "bar",
      data: {{
        labels: data.backlog.map(x => x.week),
        datasets: [{{
          label: "Cards puxados",
