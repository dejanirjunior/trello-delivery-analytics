...
CSV_PATH = Path(os.getenv("TRELLO_CSV_PATH", "/app/data/cards_enriched.csv"))
...
def _to_bool_int(value):
    ...
    return 1 if value_str in ("true", "1", "yes", "sim") else 0
...
def _has_block_label(row):
    ...
    if _to_bool_int(is_block_value) == 1:
        return 1
...
def load_cards():
    if not CSV_PATH.exists():
        print(f"Arquivo de cards não encontrado: {CSV_PATH}")
        return []
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            cards.append({
                ...
                "cliente_label": _safe_str(row.get("cliente_label")),
